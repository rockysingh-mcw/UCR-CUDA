...
void lower_upper_lemire(double *t, int len, int r, double *l, double *u)
{
    ...
    // this loop is to fill the empty u and l since we have skipped r-1 points due to the condition "if (i > r)".
...
double lb_kim_hierarchy(double *t, double *q, int j, int len, double mean, double std, double bsf = INF)
{
    ...
    double d, lb;
    double x0 = (t[j] - mean) / std;             // fetching 1st value of t and normalizing it
    double y0 = (t[(len - 1 + j)] - mean) / std; // fetching last value of t and normalizing it
    lb = dist(x0, q[0]) + dist(y0, q[len - 1]);  // distance between first point of t and q and last point of t and q
    if (lb >= bsf)
        return lb;
...
/// uo, lo: upper and envelops for the query, which already sorted.
...
/// A,B: data and query, respectivelycl
...
int main(int argc, char *argv[])
{
    ...
    /////////////////////////////////////////----READING THE FILES DATA AND QUERY-----//////////////////////////////////////////////////////////////////////////
    fp = fopen(argv[1], "r");
    if (fp == NULL)
        error(2);
    ...
    ;
    ///////////////////////////////////////////-----Reading the query and compute mean and std to z-normalise query-----/////////////////////////////////////////////////////////////
    ...
    free(Q_tmp);
    //////////////////////////////////---Uptil here we have read the query and prepare everything for it-----//////////////////////////////////////////////

    /////////////////////////////-----START TO SCAN THE DATA-----//////////////////////////////////////////////
    // Note that we read the data in chunk of size EPOCH for better speed.
    ...
    double lutime = 0;
    double lbkimtime = 0;
    double lbkeoghtime = 0;
    double lbkeogh2time = 0;
    double algotime = 0;
    double time_inside_while_before_for_loop = 0;
    while (!done)
    {
        ...
        time_inside_while_before_for_loop += duration<double>(t16 - t15).count();
        ...
        else
        {
            ...
            lutime += duration<double>(t6 - t5).count();
            ...
            for (i = 0; i < ep; i++)
            {
                ...
                if (i >= m - 1)
                {
                    ...
                    lbkimtime += duration<double>(t8 - t7).count();
                    ...
                    if (lb_kim < bsf)
                    {
                        ...
                        lbkeoghtime += duration<double>(t10 - t9).count();
                        if (lb_k < bsf)
                        {
                            ...
                            lbkeogh2time += duration<double>(t12 - t11).count();
                            if (lb_k2 < bsf)
                            {
                                ...
                                dtw_time += duration<double>(t4 - t3).count();
            ...
            algotime += duration<double>(t14 - t13).count();
        }
    }
    ...
    cout << "DTW Time : " << (double)dtw_time * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "Lemire Time : " << (double)lutime * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "LB_Kim Time : " << (double)lbkimtime * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "LB_Keogh Time : " << (double)lbkeoghtime * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "LB_Keogh2 Time : " << (double)lbkeogh2time * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "calculation time : " << (double)(dtw_time + lbkimtime + lbkeoghtime + lbkeogh2time) * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "Total Execution Time : " << duration<double>(t2 - t1).count() * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "Data Loading time from file : " << (double)(time_inside_while_before_for_loop) * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
    cout << "Algorithm Time : " << (double)algotime * 1000000 / CLOCKS_PER_SEC << " sec" << endl;
